<?php
    ...
    function movieSawGenreList(){
      ...
      while($genre = mysqli_fetch_array($genre_result)){
        echo '<button type="button" class="btn btn-default"><a href="hated_genre.php?id='
    ...
    function genreClick(){
      ...
      $movie_genre_sql = "SELECT * FROM movie_genre_list WHERE `movie_id` IN (SELECT `movie_id` FROM `user_hate_list` WHERE `userlist_id`=$user_id)";
        ...
        while($movie_genre = mysqli_fetch_array($movie_genre_result)){
          if($movie_genre['genre_id']===$_GET['id']){ 
            ...
            echo '<br><button type="button" class="btn btn-default"><a href=".movie_info.php?id='
